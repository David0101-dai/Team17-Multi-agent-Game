using UnityEngine;
public class RedHoodGroundState : RedHoodThinkState
{
    private float flipCooldownTimer = 0f;  // 反转冷却计时器
    private const float flipCooldown = 1.0f;  // 反转冷却时间（0.5秒）

    public RedHoodGroundState(FSM fsm, RedHood character, string animBoolName) : base(fsm, character, animBoolName)
    {
    }
